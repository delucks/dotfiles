...
screen 1 do
  top    [ :views, :separator, :title, :keychain, :spacer,  :center, :clock, :center, :wifi, :ipaddr,:separator, :battery ]
	bottom []
end
...
gravity :centervert,     [   0,   0, 100, 100 ], :vert
...
# Switch current view (backup keys)
...
grab "W-m", :WindowFloat

# Toggle fullscreen mode of window
grab "W-apostrophe", :WindowFull
...
# Raise window
grab "W-S-A-r", :WindowRaise

# Lower window
grab "W-S-A-l", :WindowLower
...
# Move windows
grab "W-S-h",  :WindowMoveLeft
grab "W-S-j",  :WindowMoveDown
grab "W-S-k",    :WindowMoveUp
grab "W-S-l", :WindowMoveRight

# Kill current window
grab "W-S-q", :WindowKill

# Cycle between given gravities
grab "W-o q", [ :top_left,     :top_left66,     :top_left33     ]
grab "W-o w", [ :top,          :top66,          :top33          ]
grab "W-o e", [ :top_right,    :top_right66,    :top_right33    ]
grab "W-o a", [ :left,         :left66,         :left33         ]
grab "W-o s", [ :center,       :centervert  ]
grab "W-o S-s", [ :center66,       :center33 ]
grab "W-o d", [ :right,        :right66,        :right33        ]
grab "W-o z", [ :bottom_left,  :bottom_left66,  :bottom_left33  ]
grab "W-o x", [ :bottom,       :bottom66,       :bottom33       ]
grab "W-o c", [ :bottom_right, :bottom_right66, :bottom_right33 ]

# Exec programs
grab "W-t", "urxvt"

# Mod+Fx
grab "W-F1", "~/scripts/dmenu_man"
grab "W-F2", "~/scripts/dmenu_run_enhanced"
grab "W-F3", "~/scripts/dmenu_ssh"
grab "W-F4", "~/scripts/dmenu_nmcli"
grab "W-F5", "~/scripts/rscontrol -i"
grab "W-F6", "~/scripts/rscontrol -d"
grab "W-F7", "~/scripts/dmenu_openwith"
# grab "W-F8", "~/scripts/"
# grab "W-F9", "~/scripts/"
grab "W-F10", "amixer set Speaker toggle"
# grab "W-F11", "~/scripts/"
# grab "W-F12", "~/scripts/"

# Special Keys
grab "XF86AudioRaiseVolume", "~/scripts/dvol -i 5"
grab "XF86AudioLowerVolume", "~/scripts/dvol -d 5"
grab "XF86KbdBrightnessUp", " asus-kbd-backlight up"
grab "XF86KbdBrightnessDown", " asus-kbd-backlight down"
grab "XF86TouchpadToggle", " ~/scripts/touchpadtoggle"
grab "XF86WebCam", " scrot -q 100 '%Y-%m-%d_%H-%M-%S_$wx$h.png' -e 'mv $f ~/pictures/screenshot'"
grab "XF86AudioMute", " ~/scripts/dvol -t"
grab "XF86AudioPlay", " mpc toggle"
grab "XF86AudioNext", " mpc next"
grab "XF86AudioPrev", " mpc prev"
grab "XF86AudioStop", " mpc stop"
grab "XF86Calculator", " ~/scripts/dmenu_calc"
grab "XF86Launch1", " mplayer tv://"
grab "XF86Launch6", " i3lock -c 262626"
...
tag "graphical" do
	match "violet|mirage|thunar|evince|zathura"
end
...
view "f" do
	match "editor"
	icon "~/.icons/xbm2/pin.xbm"
	icon_only false
end

view "g" do
	match "default"
	icon "~/.icons/xbm2/grid.xbm"
	icon_only false
end
...
view "vmware" do
	match "vmware"
	dynamic true
	icon "~/.icons/xbm2/pc.xbm"
end

view "games" do
	match "games"
	dynamic true
	icon "~/.icons/xbm2/game.xbm"
end

view "gimp" do
  match "gimp_.*"
  dynamic true
	icon "~/.icons/xbm2/edit1.xbm"
end
